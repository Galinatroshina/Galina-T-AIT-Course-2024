...
public class BankAccount {
    ...
    public boolean deposit(double sum){ //было столько то, положили, вернули и баланс изменился
        balance = balance + sum;
        return true;
    }
    public boolean withdraw(double sum){ //метод вычитает сумму, // если средств на счету не достаточно используем метод если
        if(sum <= balance){
            balance = balance - sum;
        }
        return false;

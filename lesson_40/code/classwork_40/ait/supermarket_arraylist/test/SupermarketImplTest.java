...
class SupermarketImplTest {
    ...
    @Test
    void findByCategory() { //itterable тоже можно использовать, это просто для примера
        ArrayList<Product> actual = (ArrayList<Product>) supermarket.findByCategory("Bread"); //безопасный кастинг
        int l = actual.size(); //перегоняем arraylist в массив
        ...
        assertEquals(expected, productsActual);
    }
    ...
    @Test
    void findByExpDate() {
        Iterable<Product> actual = supermarket.findByExpDate();
    ...
    @Test
    void skuQuantity() {
        assertEquals(5, supermarket.skuQuantity()); //спрашиваем сколько, предполагаем что 5 прдуктов
    }
}